...
export function PdfViewDialog({ attachment, onClose }: Props) {
            ...
            {!loaded && (
              <div className='absolute inset-0 flex items-center justify-center bg-card'>
                <div className='flex flex-col items-center gap-3'>
                  ...
                  <span className='text-[12px] text-muted-foreground'>Đang tải tài liệu...</span>
                </div>
              </div>
            )}
            <iframe src={attachment.url} title={attachment.name} onLoad={() => setLoaded(true)} className='w-full h-full border-0' />
          </div>
