...
export function ImageViewDialog({ attachment, onClose }: Props) {
      ...
      <DialogPrimitive.Portal>
        ...
        <DialogPrimitive.Content className='fixed inset-0 z-50 flex flex-col focus:outline-none'>
          ...
          <div className='flex-1 flex items-center justify-center p-6 overflow-hidden relative'>
            ...
            <img src={attachment.url} alt={attachment.name} onLoad={() => setLoaded(true)} className={`max-w-full max-h-full object-contain rounded-sm select-none transition-opacity duration-300 ${loaded ? 'opacity-100' : 'opacity-0'}`} />
          </div>
