...
export function ThemeTab({ selectedTheme, setSelectedTheme, selectedTweakcnTheme, setSelectedTweakcnTheme, selectedRadius, setSelectedRadius, setImportedTheme, onImportClick }: ThemeTabProps) {
  const { isDarkMode, brandColorsValues, setBrandColorsValues, applyTheme, applyTweakcnTheme, applyRadius, handleColorChange } = useThemeManager();
    ...
    <div className='p-4 space-y-6'>
      ...
      <div className='space-y-3'>
        ...
        <div className='grid grid-cols-2 gap-2'>
          <Button variant={!isDarkMode ? 'secondary' : 'outline'} size='sm' onClick={handleLightMode} className='cursor-pointer'>
            <Sun className='h-4 w-4 mr-1' />
            Light
          </Button>
          <Button variant={isDarkMode ? 'secondary' : 'outline'} size='sm' onClick={handleDarkMode} className='cursor-pointer'>
          ...
          <AccordionContent className='px-4 pb-4 pt-2 space-y-3 border-t border-border bg-muted/20'>
            {baseColors.map((color) => (
              <div key={color.cssVar} className='flex items-center justify-between'>
                <ColorPicker label={color.name} cssVar={color.cssVar} value={brandColorsValues[color.cssVar] || ''} onChange={handleColorChange} />
              </div>
            ))}
          </AccordionContent>
        </AccordionItem>
      ...
      <div className='p-4 bg-muted rounded-lg space-y-3'>
        ...
        <p className='text-xs text-muted-foreground'>
          ...
          <a href='https://tweakcn.com/editor/theme' target='_blank' rel='noopener noreferrer' className='text-primary hover:underline font-medium cursor-pointer'>
            tweakcn.com
          </a>
        </p>
        <Button variant='outline' size='sm' className='w-full cursor-pointer' onClick={() => typeof window !== 'undefined' && window.open('https://tweakcn.com/editor/theme', '_blank')}>
