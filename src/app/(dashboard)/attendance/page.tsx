...
export default function AttendancePage() {
  ...
  const attendance = useAttendance(organizationId || null);
  const employees = useEmployees(organizationId || null);
  const actions = useAttendanceMutations(organizationId || null);
  ...
  useEffect(() => { if (!assignmentEmployeeId && employees.data?.[0]) setAssignmentEmployeeId(employees.data[0].id); }, [assignmentEmployeeId, employees.data]);
  ...
  const submitAssignment = (event: FormEvent) => { event.preventDefault(); const schedule = attendance.schedules.data?.[0]; if (!schedule || !assignmentEmployeeId) return toast.error('Cần có nhân sự và lịch làm việc'); void run(() => actions.createAssignment.mutateAsync({ employeeId: assignmentEmployeeId, scheduleId: schedule.id, effectiveFrom: today() }), 'Đã phân lịch cho nhân sự'); };
  ...
  return <main className='space-y-6 p-6'>
    <header className='flex flex-wrap items-center justify-between gap-3'><div><h1 className='text-2xl font-semibold'>Chấm công</h1><p className='text-sm text-muted-foreground'>Giờ chấm công được lấy từ server.</p></div><select value={organizationId} onChange={event => setOrganizationId(event.target.value)} className='rounded border bg-background px-3 py-2'>{organizations.data.map(organization => <option key={organization.id} value={organization.id}>{organization.name}</option>)}</select></header>
