...
import { createUniqueTaskColumnId } from '@/modules/tasks/utils/taskColumns';
...
export function TaskColumnDialog({ open, existingColumns, column, onClose, onSuccess }: Props) {
  const isEdit = !!column;
  ...
  return (
    <ModalShell
      ...
      onClose={handleClose}
      size='sm'
      title={isEdit ? 'Sửa cột Kanban' : 'Thêm cột Kanban'}
    ...
    >
      <div className='px-6 py-5 space-y-4'>
        <div className='space-y-1.5'>
          ...
          <Input value={title} onChange={(e) => setTitle(e.target.value)} disabled={saving} placeholder='Ví dụ: Ready for QA' className='text-[13px]' />
        </div>
